...
class World:
    ...
    def final_read_then_commit(
        self,
        approval: Approval,
        *,
        revoke_after_final_read: bool,
    ) -> Dict[str, object]:
        ...
        if not (target_ok and external_ok):
            return {
        ...
        if revoke_after_final_read:
            self.mutate_version("compliance")

        result = self.execute_if_match_only(approval)
        return {
            "final_read_passed": True,
            "compliance_still_matches_at_commit":
                self.resources["compliance"].version
                == approval.external_versions["compliance"],
            "commit": result,
            "effect": self.resources["target"].value,
        }
...
def run_matrix() -> Dict[str, object]:
    ...
    w = World()
    a = authorize(w)
    assert verifier_passes(w, a)
    w.mutate_version("target")
    out["T2_target_race_if_match"] = {
        "result": w.execute_if_match_only(a),
        "effect": w.resources["target"].value,
    }
    ...
    out["T3_external_race_if_match_only"] = {
        "target_version_still_matches":
            w.resources["target"].version == a.target_version,
        "budget_version_matches":
            w.resources["budget"].version == a.external_versions["budget"],
        "result": w.execute_if_match_only(a),
        "effect": w.resources["target"].value,
    }
    ...
    out["T4_external_race_full_recheck"] = {
        "full_recheck_passed": all_ok,
        "effect": w.resources["target"].value,
    }

    w = World()
    a = authorize(w)
    before = verifier_passes(w, a)
    w.mutate_version("target")
    out["T5_verifier_to_commit_target_race"] = {
    ...
    w = World()
    a = authorize(w)
    out["T7a_final_read_no_race"] = w.final_read_then_commit(
        a, revoke_after_final_read=False
    )

    w = World()
    a = authorize(w)
    out["T7b_revoked_after_final_read"] = w.final_read_then_commit(
        a, revoke_after_final_read=True
    )
...
def self_test(results: Dict[str, object]) -> None:
    ...
    assert results["T4_external_race_full_recheck"]["full_recheck_passed"] is False
    ...
    assert results["T7b_revoked_after_final_read"]["compliance_still_matches_at_commit"] is False
